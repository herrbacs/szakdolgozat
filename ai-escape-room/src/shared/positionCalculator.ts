...
function coordinatesOfFT2({ perspective, width, height } : ScreenSettings, sprite : Sprite, scale: number) : Coordinate {
  const offset = 35
  return {
    X: perspective + ((width - (perspective * 2)) / 2) - ((sprite.width * scale) / 2),
